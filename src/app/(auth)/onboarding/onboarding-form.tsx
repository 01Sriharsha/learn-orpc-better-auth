...
export default function OnboardingForm() {
  ...
  // User details form
  const detailsForm = useForm<OnBoardUserSchema>({
  ...
  const otpForm = useForm<Pick<VerifyEmailOTPSchema, "code">>({
  ...
  const onboardMutation = useMutation(
    orpcQC.auth.onBoardUser.mutationOptions({
      onSuccess: (data) => {
        setStep("otp");
  ...
  const otpVerifyMutation = useMutation(
    orpcQC.auth.verifyOnboardEmailOTP.mutationOptions({
      onSuccess: (data) => {
  ...
  const onDetailsSubmit = async (data: OnBoardUserSchema) => {
    onboardMutation.mutate(data);
  };

  const onOTPSubmit = async (data: Pick<VerifyEmailOTPSchema, "code">) =>
    otpVerifyMutation.mutate({
      code: data.code,
      businessEmail: detailsForm.getValues("businessEmail"),
    });
  ...
  const isLoading = onboardMutation.isPending || otpVerifyMutation.isPending;

  return (
    <div className="relative max-w-lg mx-auto bg-white rounded-xl p-7 shadow-lg w-full z-10 space-y-4">
      <motion.div
            ...
            <Form {...detailsForm}>
              ...
              >
                ...
                >
                  ...
                  >
                    {isLoading ? (
                      <Loader>Sending verification...</Loader>
                    ) : (
                      <>
                        Continue
              ...
              >
                ...
                >
                  ...
                  <span className="font-medium text-blue-700">
                    {detailsForm.getValues("businessEmail")}
                  </span>
                </motion.div>
