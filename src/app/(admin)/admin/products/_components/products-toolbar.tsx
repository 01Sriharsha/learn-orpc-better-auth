...
export default function ProductsToolbar() {
    ...
    if (hasInitialData && debouncedSearch !== filters.keyword) {
      const performSearch = async () => {
        ...
        try {
          ...
          // Only include categoryId if it's not undefined
  ...
  }, [debouncedSearch, hasInitialData]);
  ...
  const handleCategoryFilter = useCallback(
    async (categoryId: string) => {
      ...
      try {
        ...
        // Only include categoryId if it's not empty/undefined
        if (categoryId) {
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row lg:flex-row gap-3 flex-1 lg:max-w-4xl">
            ...
            <div className="flex flex-col xs:flex-row sm:flex-row gap-3 lg:gap-3">
              ...
              {/* Category Filter */}
              <motion.div
                className="flex-1 min-w-0 lg:min-w-[200px]"
                whileHover={{ scale: 1.02 }}
                transition={{ duration: 0.2 }}
              >
                <CategorySelect
                  ...
                  className="w-full border-slate-200 focus:border-blue-400 focus:shadow-lg focus:shadow-blue-500/10 transition-all duration-200"
                />
              </motion.div>
            </div>
          </div>
